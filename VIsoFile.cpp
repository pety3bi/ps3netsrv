    ...
    *size = statbuf.file_size;
    ...
    for( int i = 1; ; i++ ) {
        ...
        *size += statbuf.file_size;
        ...
        if( i > 1 ) {
            if( prev_size & 0x7FF ) {
                syslog( LOG_ERR, "666XX file must be multiple of sector, except last fragment. (file=%s)", file );
            }
        }
...
static int get_ucs2_from_utf8( const unsigned char* input, const unsigned char** end_ptr )
{
    ...
    *end_ptr = input;
    ...
    syslog( LOG_ERR, "Fatal error: windows version of server currently doesn't support non-ansi characters.\n, character giving error: 0x%02x", *input );
    return -1;
}
...
static void get_rand( void* bfr, uint32_t size )
{
    ...
    if( !CryptAcquireContext( &hProv, NULL, NULL, PROV_RSA_FULL, CRYPT_VERIFYCONTEXT ) ) {
        syslog( LOG_ERR, "Error aquiring crypt context." );
    }

    if( !CryptGenRandom( hProv, size, ( BYTE* )bfr ) ) {
        syslog( LOG_ERR, "Error getting random numbers." );
    }
...
static bool get_title_id( const char* dir, char* title_id )
{
    ...
    if( !FD_OK( fd ) ) {
        syslog( LOG_ERR, "Cannot find %s", sfo_path );
        return false;
    }

    if( parse_param_sfo( fd, "TITLE_ID", title_id ) != 0 ) {
        syslog( LOG_ERR, "TITLE_ID not found" );
        return false;
    }
        ...
        while( dirList ) {
            ...
            while( fileList ) {
                ...
                if( vFilePtr >= fStart && vFilePtr < fEndSector ) {
                    if( fileList->multipart ) {
                        syslog( LOG_ERR, "Sorry no support for 666 files. I have the feeling that your game is about to crash ^_^" );
                        return r;
                    }
                    ...
                    if( vFilePtr < fEnd ) {
                        ...
                        if( !FD_OK( fd ) ) {
                            syslog( LOG_ERR, "VISO: file %s cannot be opened!", fileList->path );
                            return r;
                        }
                        ...
                        if( this_r < 0 ) {
                            syslog( LOG_ERR, "VISO: read_file failed on %s", fileList->path );
                            return r;
                        }

                        if( this_r != to_read ) {
                            syslog( LOG_ERR, "VISO: read on file %s returned less data than expected (file modified?)", fileList->path );
                            return r;
                        }
